...
from domains.workflows.prompt import initialize_entity_extraction_prompt, initialize_summary_prompt, initialize_image_analysis_prompt, initialize_multi_image_analysis_prompt
...
async def consolidate_image_analyses(image_analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
    try:
        ...
        logger.debug("Image analyses for consolidation: {}".format(analyses_text))
        all_analyses = "\n\n".join(analyses_text)

        llm = initialize_chat_model()
        multi_image_prompt = initialize_multi_image_analysis_prompt()

        chain = multi_image_prompt | llm | JsonOutputParser()

        logger.debug("Invoking consolidation chain with image summaries: {}".format(image_analyses))
...
async def process_images(state: OrchestratorState) -> Dict[str, Any]:
    try:
        ...
        for img_path in images_path:
            try:
                ...
                prompt = summary_generation_prompt(
                    ...
                    template=image_analysis_prompt.template
                )

                response = await llm.ainvoke(prompt)

                logger.debug("Image analysis response summary: {}".format(response.content))
        ...
        # If we have multiple images, consolidate the analyses
