...
class ImageLoader:
    ...
    # Base64 encoding increases size by ~33%, so we need a lower threshold
    # Using 3.75MB as the threshold to ensure encoded size stays under 5MB
    MAX_IMAGE_SIZE_BEFORE_ENCODING = int(MAX_IMAGE_SIZE_BYTES * 0.75)
    ...
    def resize_image_if_needed(self) -> Union[bytes, None]:
        ...
        if file_size <= self.MAX_IMAGE_SIZE_BEFORE_ENCODING:
            logger.debug(f"Image size {file_size} bytes is within limits for encoding, no resize needed")
            return None

        logger.info(f"Image size {file_size} bytes exceeds {self.MAX_IMAGE_SIZE_BEFORE_ENCODING} bytes limit (to ensure encoded size stays under {self.MAX_IMAGE_SIZE_BYTES} bytes), resizing")
        ...
        try:
            ...
            while True:
                ...
                if current_size <= self.MAX_IMAGE_SIZE_BEFORE_ENCODING:
    ...
    def encode_image_to_base64(self) -> str:
        ...
        try:
            ...
            if encoded_size > self.MAX_IMAGE_SIZE_BYTES:
                logger.warning(f"Encoded image size {encoded_size} bytes still exceeds {self.MAX_IMAGE_SIZE_BYTES} bytes limit. Forcing resize.")
                # Force more aggressive resize
                ...
                output = io.BytesIO()
                img_format = self.image_type.upper() if self.image_type.upper() != 'JPG' else 'JPEG'
                img_resized.save(output, format=img_format, quality=70)
                output.seek(0)
                encoded_data = base64.b64encode(output.getvalue())
                logger.info(f"Forced resize to {new_width}x{new_height} with quality 70, new encoded size: {len(encoded_data)} bytes")
